...
for ii=1:76
    ...
    lags(lags_sum(ii)+1)=lags(lags_sum(ii)+1)+data(1,jj,ii,index(pulse),pulse)+1j*data(2,jj,ii,index(pulse),pulse);
end
x(:,jj)=lags./lags_rep;
x(1,jj)=zero_lag_power(jj,index(pulse),pulse);
end
x=x/96;
...
rrrr((48:95),:)=x;
for looloo=1:47
rrrr(looloo,:)=real(x(49-looloo,:))-1j*imag(x(49-looloo,:));
end
